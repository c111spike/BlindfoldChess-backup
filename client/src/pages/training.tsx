...
function VoiceMoveMasterGame({ onBack, onComplete, stats, onGameStateChange }: VoiceMoveMasterGameProps) {
  ...
  const processVoiceInput = useCallback((transcript: string) => {
    ...
    if (input.includes('resign') || input.includes('quit')) {
      ...
      speak("Are you sure you want to resign? Say yes or no.");
      return;
    }
    ...
    if (whereIsMatch) {
      ...
      if (pieceType) {
        ...
        if (squares.length > 0) {
          speak(`Your ${pieceName} is on ${squares.slice(0, 3).join(', ')}`);
        } else {
          speak(`You don't have a ${pieceName}`);
        }
      }
      return;
    }
    ...
    if (input.includes('eval') || input.includes('evaluate')) {
      ...
      if (material > 0) {
        speak(`White is up ${material} points`);
      } else if (material < 0) {
        speak(`Black is up ${Math.abs(material)} points`);
      } else {
        speak('Position is equal');
      }
      return;
    }
    ...
    if (legalMovesMatch) {
      ...
      if (pieceType) {
        const moves = position.chess.moves({ verbose: true })
          ...
          .map(m => m.san);
        if (moves.length > 0) {
          speak(`${pieceName} can play ${moves.join(', ')}`);
        } else {
          speak(`No legal moves for ${pieceName}`);
        }
      }
      return;
    }
